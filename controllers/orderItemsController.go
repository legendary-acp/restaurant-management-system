...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while listing order itmes"})
			return
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	if result.All(ctx, &OrderItems); err != nil {
		panic(err)
	}
...
func UpdateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if orderItem.Quantity != nil {
			updateObj = append(updateObj, bson.E{"unit_price", orderItem.Quantity})
		}
		if orderItem.Food_id != nil {
			updateObj = append(updateObj, bson.E{"unit_price", orderItem.Food_id})
		}
		orderItem.Updated_at, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
